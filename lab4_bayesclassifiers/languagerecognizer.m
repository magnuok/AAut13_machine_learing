...
% Classify texts according to the language in which they are written.
	...
	while 1			% Main loop: Read the input text and then classify it
		...
		for languageindex = 1:numel(languages)          % Loop through the available languages
			...
			% ***************************************************************************************************************
			% Add here any initialization code that is to be 
            % performed once for each language, before the loop on trigrams.

			currentscore = 0; 
            %Initialize the score counter for the current language
			...
			for trigramindex = 1:numel(text)-2;					% Loop through all the trigrams of the input text
				...
				% *****************************************************************
                currentscore = currentscore + trigramcount;
                % Adds the trigramcount for the current triagram to the current score
            ...
            scores(languageindex) = currentscore*1000/total_counts(languageindex);
            % Score for the current language is given by the current score
            % of all triagrams. 
         
            % The score is divided by the count of the language to take
            % into account the different sizes of training data pr language
            
            %Multiply by 1000 to get a nicer score for small words/sentences.
